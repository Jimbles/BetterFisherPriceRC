...
byte Data_Array[numData]={B00000001,B00000010,B00000100,B00001000};
...
void setup() {
  ...
  //radio.setPALevel(RF24_PA_MIN); //can be MIN LOW HIGH or MAX
...
void loop() {
  ...
  unsigned char data = Data_Array[curData];
  curData++;
  ...
  SENDOK=radio.write( &data, sizeof(unsigned char) );
  
  if (!SENDOK) {
    Serial.println("No acknowledgement of transmission - receiving radio device connected?");    
  }

  digitalWrite(SENDOK_PIN,SENDOK);
